...
class SetSpeedJob(BaseJob):
  """Sets the target's requested speed (CMD_SET_SPEED)."""
...
class SetPidJob(BaseJob):
  """Sets the target's PID tunings (CMD_SET_PID)."""
...
class RequestStatusJob(BaseJob):
  """Polls the target for live telemetry (CMD_REQ_STAT)."""
...
class RequestSettingsJob(BaseJob):
  """Reads back the target's currently active PID tunings and speed
  (CMD_REQ_SETTINGS)."""
