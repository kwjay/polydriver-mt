...
class TestLinkLayer(unittest.TestCase):
  ...
  def test_parser_recovers_from_corrupt_length_byte(self):
    """Regression test: a garbage length byte larger than MAX_PAYLOAD_LEN
    must not permanently wedge the parser (previously it would stall in
    READ_PAYLOAD forever since the payload buffer could never reach the
    corrupted target length).
    """
